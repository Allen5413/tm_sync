...
public class CountStudentOrderTMForSelectCourseDAOImpl extends BaseQueryDao
    ...
    public List<Object[]> findListByWhere(Map<String, String> paramsMap, Map<String, Boolean> sortMap) {
        ...
        sql.append("WHERE sc.student_code = s.code and tmc.course_code = sc.course_code and tmc.teach_material_id = tm.id and s.spot_code = ir.spot_code AND tm.state = 0 AND sc.semester_id = ? ");
        //sql合并
        ...
        sql.append("WHERE sc.student_code = s.code and stm.buy_course_code = sc.course_code and stmtm.set_teach_material_id = stm.id and stmtm.teach_material_id = tm.id and s.spot_code = ir.spot_code AND tm.state = 0 AND sc.semester_id = ? ");
