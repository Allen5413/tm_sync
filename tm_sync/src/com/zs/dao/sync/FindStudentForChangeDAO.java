...
public interface FindStudentForChangeDAO extends EntityJpaDao<Student,Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT " +
        ...
        "or s.sex != case when st.sex = 1 then 0 when st.sex = 2 then 1 else st.sex end " +
        ...
        "or s.type != CASE WHEN st.type = 'H' THEN 3 WHEN st.type = 'Y' THEN 4 WHEN st.type = 'W' THEN 0 WHEN st.type = 'Z' THEN 1 WHEN st.type = 'K' THEN 2 WHEN st.type = 'B' THEN 5 WHEN st.type = 'J' THEN 6 WHEN st.type = 'P' THEN 7 ELSE st.type END " +
     ...
     * @return
    */
    @Query(nativeQuery = true, value = "select st.* from sync_student_temp st where not EXISTS (select * from sync_student s where st.code = s.code)")
    public List<Object[]> findNewStudent();
}
